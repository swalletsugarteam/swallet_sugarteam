...
if (sendPage) {
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        sendButton.addEventListener("click", async () => {
            ...
            try {
                const response = await fetch('http://13.49.66.190:4000/api/sendCrypto', {
                    method: 'POST',
                    headers: {
...
if (receivePage) {
    document.querySelectorAll('.select_currecy_popup .asset').forEach(asset => {
        asset.addEventListener('click', function() {
            ...
            fetch(`http://13.49.66.190:4000/api/getAddress/${currencyNameSmall}`)
...
if (mainPage) {
    async function fetchTransactions(userId) {
        try {
            const response = await fetch(`http://13.49.66.190:4000/api/transactions?username=${userId}`);
...
function updateStep(step) {
    fetch('http://13.49.66.190:4000/api/updateStep', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
if (startPage) {
    ...
    fetch(`http://13.49.66.190:4000/api/user/${user.id}`)
    .then(response => response.json())
    .then(userData => {
        if (userData.register_step === 1) {
            ...
            document.querySelector("#createWalletBtn").addEventListener("click", () => {
                ...
                fetch('http://13.49.66.190:4000/api/createWallet', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
                    ...
                    if (activeIndex === dots.length) {
                        if (!isFirstPinEntered) {
                            ...
                            if (firstPin === secondPin) {
                                fetch('http://13.49.66.190:4000/api/updatePin', {
                                    method: 'POST',
                                    headers: {
                                        'Content-Type': 'application/json'
                                    },
...
document.addEventListener("DOMContentLoaded", () => {
    if (document.querySelector(".coinBalancePage")) {
        ...
        fetch(`http://13.49.66.190:4000/api/transactions?username=${user.username}`)
            .then(response => response.json())
            .then(transactions => {
